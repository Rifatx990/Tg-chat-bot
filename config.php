...
if (!file_exists($chatSettingsFile)) {
    file_put_contents($chatSettingsFile, json_encode(['auto_delete_days'=>1], JSON_PRETTY_PRINT));
}
...
function cleanOldChats() {
    ...
    foreach ($chat as $chat_id => &$chatData) {
        if(isset($chatData['messages'])){
            $chatData['messages'] = array_filter($chatData['messages'], fn($msg) => ($now - $msg['timestamp']) <= $max_age);
        }
    }
    saveChat($chat);
}
